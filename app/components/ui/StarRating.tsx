...
export function StarRating({ rating, count, size = 'md', className = '', showCount = true }: StarRatingProps) {
    ...
    return (
        <div className={`flex items-center gap-1 ${className}`}>
            ...
            {showCount && count !== undefined && (
                <span className="text-xs text-brand-navy/60 font-medium ml-1">({count})</span>
            )}
        </div>
    );
}
