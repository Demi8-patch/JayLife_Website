...
export function RitualGrid({ rituals }: RitualGridProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {rituals.map((ritual) => (
        <Link key={ritual.handle} to={`/ritual/${ritual.handle}`} className="group relative">
          {/* Card Container */}
            ...
            <div className="h-80 w-full relative overflow-hidden bg-warm-sunrise-offwhite">
              {ritual.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="font-display font-bold text-4xl text-warm-sunrise-charcoal/20">
                    {ritual.title}
                  </span>
                </div>
              )}
              ...
              {ritual.badges && ritual.badges.length > 0 && (
                <div className="absolute top-3 left-3 flex flex-col gap-2">
                  {ritual.badges.map((badge: string) => (
                    <span
                      key={badge}
                      className="bg-warm-sunrise-charcoal text-white px-2 py-1 text-xs font-bold uppercase rounded"
                    >
                      {badge}
                    </span>
                  ))}
              ...
              <div className="mb-4">
                {ritual.compareAtPrice ? (
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-xl text-warm-sunrise-charcoal">
                      ${ritual.price}
                    </span>
                    <span className="text-sm line-through text-warm-sunrise-charcoal/40">
                      ${ritual.compareAtPrice}
                    </span>
                  </div>
                ) : (
